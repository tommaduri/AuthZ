// Package auth provides token revocation using Redis
package auth
...
type RevocationStore interface {
	...
	RevokeTokenBatch(ctx context.Context, tokens map[string]time.Time) error
	IsRevokedBatch(ctx context.Context, jtis []string) (map[string]bool, error)
}
...
// NewRedisRevocationStore creates a new Redis-based revocation store
...
// RevokeToken adds a JWT token to the Redis blacklist
// Key format: blacklist:jwt:{jti}
// TTL matches token expiration to ensure automatic cleanup
func (r *RedisRevocationStore) RevokeToken(ctx context.Context, jti string, expiresAt time.Time) error {
	if jti == "" {
	...
	// Use Redis key format: blacklist:jwt:{jti}
...
func (r *RedisRevocationStore) RevokeTokenBatch(ctx context.Context, tokens map[string]time.Time) error {
	if len(tokens) == 0 {
		return nil
	}
...
func (r *RedisRevocationStore) IsRevokedBatch(ctx context.Context, jtis []string) (map[string]bool, error) {
	if len(jtis) == 0 {
...
// GetBlacklistSize returns the number of revoked tokens in Redis
func (r *RedisRevocationStore) GetBlacklistSize(ctx context.Context) (int64, error) {
	var cursor uint64
	var count int64

	for {
		keys, nextCursor, err := r.client.Scan(ctx, cursor, "blacklist:jwt:*", 100).Result()
		if err != nil {
...
// Note: Redis TTL automatically handles cleanup, but this can be used for maintenance
func (r *RedisRevocationStore) ClearExpired(ctx context.Context) (int64, error) {
	var cursor uint64
	var deleted int64
	now := time.Now().Unix()
